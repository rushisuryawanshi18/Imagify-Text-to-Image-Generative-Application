...
const BuyCredit = () => {
  ...
  const {loadCreditsData, user, token, setShowLogin } = useContext(AppContext)
  ...
  const paymentStripe = async (planId) => {
    try {
      ...
      const { data } = await axios.post('http://localhost:4000/api/user/pay-stripe', { planId }, { headers: { token } })
      if (data.success) {
